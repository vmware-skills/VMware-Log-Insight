## v1.9.0 — the MCP instructions name the configured targets
...
new install — this server now reports `Configured targets: could not be read (FileNotFoundError) — run `vmware-log-insight doctor`.` instead of falling silent. A client shown no listing cannot
...
Every CLI command that reaches Log Insight now passes the same `guard()` as its MCP twin and writes one
audit row under that tool's name (7 commands, `@audited`). Commands that reach nothing remote say so, with a reason (2, `@cli_local`).
...
## v1.8.16 — OpenClaw can load the skill
...
sets `VMWARE_LOG_INSIGHT_CONFIG` to `~/.vmware-log-insight/config.yaml`, but MCP clients pass env values verbatim and the
...
## v1.8.14 — the test suite runs on a non-UTF-8 machine, and the guardrail tests with it
...
## v1.8.13 — text I/O declares its codec

Text I/O now declares its codec. Reading a UTF-8 file with no `encoding=` uses
the machine's locale codec, which on the cp936 hosts this family is aimed at is
not UTF-8. A gate fails the suite on any undeclared read from here on.
...
(`/plugin install vmware-log-insight@vmware-skills`). The skill and its MCP server arrive in
...
## v1.8.9 — moved to vmware-skills org + MCP Registry namespace io.github.vmware-skills/vmware-log-insight
