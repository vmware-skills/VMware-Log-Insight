...
reproduced here: with ``VMWARE_LOG_INSIGHT_CONFIG`` set,
``vmware-log-insight doctor`` reported "Config file PASS" and "Config
parse PASS — 1 target(s) configured" against ``~/.vmware-log-insight/config.yaml``,
while ``load_config()`` — the call every tool makes — raised
``FileNotFoundError`` on the path in the variable.

``load_config`` resolves ``config_path or $VMWARE_LOG_INSIGHT_CONFIG or
CONFIG_FILE``. ``run_doctor`` resolved ``config_path or CONFIG_FILE``, skipping
the env var, and then passed that path *explicitly* to ``load_config`` — which
suppressed the env var there too. So the doctor did not merely check a different
file, it made itself consistent with the wrong one, and produced a green report
for a configuration nothing else would ever read.
...
from vmware_log_insight import config as cfg
from vmware_log_insight import doctor as doc
...
def sandbox(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("VMWARE_LOG_INSIGHT_CONFIG", raising=False)
...
def test_the_env_var_decides_which_file_is_resolved(sandbox, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VMWARE_LOG_INSIGHT_CONFIG", str(elsewhere))
...
def test_an_explicit_path_still_beats_the_env_var(sandbox, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VMWARE_LOG_INSIGHT_CONFIG", str(tmp_path / "ignored.yaml"))
...
def test_doctor_fails_when_the_env_var_points_at_a_missing_file(
    ...
    monkeypatch.setenv("VMWARE_LOG_INSIGHT_CONFIG", str(missing))
    ...
    assert "1target(s)configured" not in out, (
        ...
        "call raises FileNotFoundError on the path in $VMWARE_LOG_INSIGHT_CONFIG"
    )
...
def test_the_mcp_server_does_not_keep_its_own_copy_of_the_precedence():
    """The third copy. The MCP server read the variable itself and passed the
    result down explicitly — the same duplication, and the reason a change to
    the rule would have had to be made in three places to take effect."""
    from vmware_log_insight.mcp_server import _shared as server
